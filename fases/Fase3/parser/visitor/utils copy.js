import Tokenizer from "./Translator.js"
...
export const renderProduction = (node, grammar) => {
    if (grammar.isFisrtRule) {
        grammar.isFisrtRule = false;  
        let index = grammar.pendingRules.indexOf(node.id);
        if (index !== -1) {
            grammar.pendingRules.splice(index, 1);
        }
        grammar.nameProduction = node.alias? node.alias : '"'+node.id+'"';
        return `
        ${node.expr.accept(grammar)}
    ...
    if (grammar.calledRules.includes(node.id) && grammar.pendingRules.includes(node.id)) {
        let index = grammar.pendingRules.indexOf(node.id);
        if (index !== -1) {
            grammar.pendingRules.splice(index, 1);
        }
        grammar.nameProduction = node.alias? node.alias : '"'+node.id+'"';
        return node.expr.accept(grammar);
    }
    return '';
}